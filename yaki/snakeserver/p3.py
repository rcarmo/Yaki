...
# Modified by Rui Carmo on 2010-04-10
...
for typecode in 'IL':
  if len(array(typecode, [0]).tostring()) == 4:
    ...
    raise RuntimeError("Neither 'I' nor uint32 are unsigned 32-bit integers.")

class CryptError(Exception): pass
def _hash(str): return hashlib.sha1(str).digest()
...
def p3_encrypt(plain,key):
  ...
  stream = array(uint32, plain+'0000'[n&3:]) # pad to fill 32-bit words
...
def _hmac(msg, key):
  ...
  return hashlib.sha1(ko+hashlib.sha1(ki+msg).digest()).digest()
